...
class HttpObserver {
    ...
    void execute(Observable<Object> observable, CompositeSubscription mCompositeSubscription){
        if(null == mCompositeSubscription){
            subscribe(observable);
        }else {
            mCompositeSubscription.add(subscribe(observable));
        }
    }

    void execute(Observable<Object> observable){
        this.execute(observable,null);
    }

    private Subscription subscribe(Observable<Object> observable){
        return observable.subscribeOn(Schedulers.io()).map(new Func1<Object, Object>() {
            @Override
